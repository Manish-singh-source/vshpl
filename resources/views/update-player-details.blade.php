            ...
            <div class="row justify-content-center">
                <div class="col-12 col-sm-10 col-md-8 col-lg-6">
                    ...
                    <div class="form-card">
                        ...
                        <form action="{{ route('update.user.registration', $registration->id) }}" method="POST"
                            id="loginForm" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <!-- Profile Image -->
                            @if ($registration->profile_image != '' && $registration->profile_image != null)
                                {{-- In center with border radios 50 % --}}
                                <div class="mb-3">
                                    <div class="d-flex justify-content-center">
                                        <img src="{{ asset($registration->profile_image) }}" alt="Profile Image" class="rounded-circle"
                            ...
                            <div class="mb-3">
                                <input type="text" class="form-control" id="full_name" name="full_name"
                                    placeholder="Enter your full name" value="{{ $registration->full_name }}" readonly>
                            </div>

                            <!-- Flat / House Number -->
                            <div class="mb-3">
                                <input type="text" class="form-control" id="house_number" name="house_number"
                                    placeholder="Enter your flat/house number" value="{{ $registration->house_number }}"
                                    readonly>
                            </div>
                            ...
                            <div class="mb-3">
                                <input type="text" class="form-control" id="wing" name="wing"
                                    placeholder="Enter your wing" value="{{ $registration->wing }}" readonly>
                            </div>

                            <!-- T Shirt Size -->
                            <div class="mb-3">
                                <label for="tshirt_size" class="form-label">T-Shirt Size</label>
                                <select id="tshirt_size" name="tshirt_size" class="form-select" required>
                                    <option value="">Select size</option>
                                    <option value="XS" {{ $registration->tshirt_size == 'XS' ? 'selected' : '' }}>XS
                                    </option>
                                    <option value="S" {{ $registration->tshirt_size == 'S' ? 'selected' : '' }}>S
                                    </option>
                                    <option value="M" {{ $registration->tshirt_size == 'M' ? 'selected' : '' }}>M
                                    </option>
                                    <option value="L" {{ $registration->tshirt_size == 'L' ? 'selected' : '' }}>L
                                    </option>
                                    <option value="XL" {{ $registration->tshirt_size == 'XL' ? 'selected' : '' }}>
                                        XL</option>
                                    <option value="XXL" {{ $registration->tshirt_size == 'XXL' ? 'selected' : '' }}>
                                        XXL</option>
                                </select>
                            </div>
